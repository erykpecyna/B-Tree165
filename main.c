...
{ 
    //This was used to detremine node and leaf size
    // printf("The page size is %d\n", getpagesize());
    // printf("The node size is %ld\n", sizeof(node));
    // printf("The leaf size is %ld\n", sizeof(leaf));

	int opt; 
    STORAGECXT_t *stg = NULL;
		...
		{ 
			case 'f': 
                ...
                while(fgets(fileReadBuffer, 1023, fp)){
                    parseRouteQuery(fileReadBuffer, &stg);
                }
